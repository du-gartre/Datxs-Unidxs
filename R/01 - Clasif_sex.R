...
# 01 Clasificación de sexos por servidorx públicx
#
...
v_nom_uni <- df_nom_unisex$nombre
...
df_agosto_sex <- df_agosto2 %>%
    ...
    mutate(unisex = if_else(condition = (N1 %in% v_nom_uni), true = 1, false = 0),
           ...
           sex = case_when((N1 %in% v_nom_muj) ~ "M",
                           (N1 %in% v_nom_hom) ~ "H",
                           # Clasificamos con el segundo nombre a las personas que tienen nombre unisex en el primer nombre.
                           ((N1 %in% v_nom_uni) & (N2 %in% v_nom_muj)) ~ "M", 
                           ((N1 %in% v_nom_uni) & (N2 %in% v_nom_hom)) ~ "H")) %>% 
